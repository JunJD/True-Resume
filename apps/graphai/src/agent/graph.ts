...
async function chat_node(state: AgentState, config: RunnableConfig) {
  ...
  const DASHSCOPE_API_KEY = process.env.OPENAI_API_KEY; // 定义Dashscope API密钥
  const DASHSCOPE_BASE_URL = process.env.OPENAI_BASE_URL; // 定义Dashscope API基础URL
  
  console.log("DASHSCOPE_API_KEY==>", DASHSCOPE_API_KEY);
  console.log("DASHSCOPE_BASE_URL==>", DASHSCOPE_BASE_URL);
  
  const model = new ChatOpenAI({
    modelName: "qwen-max",
    temperature: 0.7,
    openAIApiKey: DASHSCOPE_API_KEY,
    configuration: {
